...
type Props = NativeStackScreenProps<RootStackParamList, 'DMConversation'>

export default function DMConversationScreen() {
  const route = useRoute<Props['route']>()
  const navigation = useNavigation<NativeStackNavigationProp<RootStackParamList>>()
  const { conversationId, otherName, otherUsername } = route.params
  const { messages, setMessages, addMessage } = useDMStore()
  ...
  useEffect(() => {
    navigation.setOptions({ title: otherName, headerBackTitle: 'Mesajlar' })
      ...
      <View style={[s.msgRow, !grouped && s.msgRowFirst]}>
        {!grouped ? (
          <View style={[s.avatar, isMe && s.avatarMe]}>
            <Text style={s.avatarText}>{item.sender.displayName[0].toUpperCase()}</Text>
          </View>
        ) : <View style={s.avatarSpacer} />}
        <View style={s.msgContent}>
          {!grouped && (
            <View style={s.msgMeta}>
              <Text style={[s.msgAuthor, isMe && s.msgAuthorMe]}>{item.sender.displayName}</Text>
    ...
    <KeyboardAvoidingView style={s.container} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={90}>
      {dmMessages.length === 0 ? (
        <View style={s.emptyContainer}>
          <Text style={s.empty}>{otherName} (@{otherUsername}) ile konuşmanın başlangıcı. Merhaba de!</Text>
        </View>
      ) : (
        <FlatList
          ref={flatRef}
      ...
      <View style={s.inputRow}>
        <TextInput
          style={s.input}
          placeholder={`${otherName} kullanıcısına yaz...`}
